...
from config import MODEL_NAME, API_KEY, VERIFY_ACTIONS, VERIFICATION_TIMEOUT
...
class OSAgent:
    ...
    def log(self, message, speak_it=False):
        print(message)
            ...
            if action == "CLICK":
                ...
                if coords:
                    ...
                    if success:
                        ...
                        if VERIFY_ACTIONS:
                            ...
                            verify_result = verifier.verify_action(f"Clicked at {coords}", "Screen should have changed after click")
                            if verify_result.get('success'):
                                self.log(f"✅ Click verified: {verify_result.get('what_happened', 'success')}", speak_it=True)
                            else:
                                self.log(f"⚠️ Click may not have worked: {verify_result.get('what_happened', 'unknown')}", speak_it=True)
                    else:
                        history.append(f"CLICK FAILED at {coords}")
                        self.log("Click failed", speak_it=True)
                ...
                if text:
                    ...
                    if success:
                        ...
                        if VERIFY_ACTIONS:
                            time.sleep(VERIFICATION_TIMEOUT)
                            verify_result = verifier.verify_action(f"Typed '{text}'", f"Text '{text}' should appear on screen")
                            if verify_result.get('success'):
                                self.log(f"✅ Typing verified", speak_it=True)
                            else:
                                self.log(f"⚠️ Typing may not have worked", speak_it=True)
                    else:
                        history.append(f"TYPE FAILED '{text}'")
                        self.log("Typing failed", speak_it=True)
                ...
                if key:
                    ...
                    if success:
                        ...
                        # Vision verification for important keys
                        if VERIFY_ACTIONS and key.lower() in ['enter', 'return', 'tab']:
                            time.sleep(VERIFICATION_TIMEOUT)
                            verify_result = verifier.verify_action(f"Pressed '{key}'", "Screen should respond to key press")
                            if verify_result.get('success'):
                                self.log(f"✅ Key press verified", speak_it=True)
                            else:
                                self.log(f"⚠️ Key press may not have worked", speak_it=True)
                    else:
                        history.append(f"PRESS FAILED '{key}'")
                        self.log("Key press failed", speak_it=True)
                ...
                self.log("✅ Task completed successfully!", speak_it=True)
                break
